 ...
 *
 * Frontend Axios interceptor converts camelCase → snake_case, so this handler
 * receives `{ code, code_verifier }` and forwards the same shape to Laravel.
 *
 * TODO(backend): Implement `POST /api/auth/google/callback` on Laravel:
 *   - Accept `{ code, code_verifier }`
 *   - Exchange code with Google token endpoint using the verifier
 *   - Verify ID token, upsert/lookup user, issue Sanctum token
 *   - Return `{ data: { user, token, accessStatus, ... } }` matching the Lark response shape
 * When ready, set `OAUTH_GOOGLE_ENABLED=true` in the environment.
 */
export async function POST(request: NextRequest) {
  if (process.env.OAUTH_GOOGLE_ENABLED !== 'true') {
    return NextResponse.json(
      {
        message:
          'Google OAuth backend is not implemented yet. Set OAUTH_GOOGLE_ENABLED=true once the Laravel route is live.',
      },
      { status: 501 },
    )
  }
  ...
  try {
    ...
    try {
      ...
      console.error('Auth Google callback: cannot reach Laravel CSRF endpoint', fetchErr)
    ...
    try {
      callbackRes = await fetch(`${LARAVEL_API_URL}/api/auth/google/callback`, {
        method: 'POST',
        headers: {
      ...
      console.error('Auth Google callback: cannot reach Laravel callback endpoint', fetchErr)
      return NextResponse.json({ message: 'Auth server unreachable' }, { status: 502 })
    }
    ...
    try {
      ...
      console.error('Auth Google callback: Laravel response was not JSON', err)
      return NextResponse.json(
    ...
    if (!token || typeof token !== 'string') {
      return NextResponse.json(
        { message: 'Google callback succeeded but no token returned' },
        { status: 502 },
      )
    }
    ...
    if (payload.data.user == null) {
      return NextResponse.json(
        { message: 'Google callback succeeded but user payload was missing' },
        { status: 502 },
      )
    }
    ...
    console.error('Auth Google callback route error:', err)
